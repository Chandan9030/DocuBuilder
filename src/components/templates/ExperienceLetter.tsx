...
const ExperienceCertificateTemplate: React.FC<ExperienceCertificateTemplateProps> = ({ formData, templateContent }) => {
  ...
  const formattedStartDate = startDate ? formatCustomDate(startDate) : formatCustomDate(new Date().toISOString().split('T')[0]);
  const formattedEndDate = endDate ? formatCustomDate(endDate) : formatCustomDate(new Date().toISOString().split('T')[0]);
  const currentDate = formatCustomDate(new Date().toISOString().split('T')[0]);
