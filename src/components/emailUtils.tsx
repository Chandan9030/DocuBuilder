...
export async function sendSingleDocumentByEmail(
  ...
  const safeName = (formData.employeeName || formData.recipientName || formData.studentName)?.replace(/[^a-zA-Z0-9]/g, '_') || 'document';
...
console.log(`Generated PDF for ${formData.recipientName}, base64 length: ${pdfBase64.length}, estimated size: ${(pdfBase64.length * 0.75 / 1024).toFixed(2)} KB`);
  ...
  try {
    const response = await fetch('http://localhost:3001/api/send-document', {
      ...
      body: JSON.stringify({
        recipientEmail,
        recipientName: formData.recipientName || formData.employeeName || formData.studentName || 'Recipient',
...
export async function sendDocumentsByEmail(
  ...
  if (employeesWithoutEmail.length > 0) {
    const names = employeesWithoutEmail
      .map(s => s.recipientName || s.employeeName || s.studentName || 'Unknown')
      .join(', ');
...
//   const templateLabel = templateOptions.find(opt => opt.value === templateType)?.label.replace(/\s+/g, '_') || 'Document';
//   const safeName = (templateType === 'paymentReceipt' ? formData.studentName : formData.employeeName || formData.recipientName)?.replace(/[^a-zA-Z0-9]/g, '_') || 'document';
//   const filename = `${templateLabel}_${safeName}.pdf`;
...
//       body: JSON.stringify({
//         recipientEmail,
//         recipientName: templateType === 'paymentReceipt' ? formData.studentName : formData.recipientName || formData.employeeName || 'Recipient',
//         subject: `${templateLabel} - ${templateType === 'paymentReceipt' ? formData.studentName : formData.recipientName || formData.employeeName || 'Recipient'}`,
//         pdfBase64,
//         pdfFilename: filename,
//       }),
//     });
...
//       const templateLabel = templateOptions.find(opt => opt.value === selectedTemplate)?.label.replace(/\s+/g, '_') || 'Document';
//       const safeName = (selectedTemplate === 'paymentReceipt' ? data.studentName : data.employeeName || data.recipientName)?.replace(/[^a-zA-Z0-9]/g, '_') || 'document';
//       const filename = `${templateLabel}_${safeName}_${index + 1}.pdf`;
...
//           body: JSON.stringify({
//             recipientEmail: data.email,
//             recipientName: selectedTemplate === 'paymentReceipt' ? data.studentName : data.recipientName || data.employeeName || 'Recipient',
//             subject: `${templateLabel} - ${selectedTemplate === 'paymentReceipt' ? data.studentName : data.recipientName || data.employeeName || 'Recipient'}`,
//             pdfBase64,
//             pdfFilename: filename,
//           }),
//         });
...
//         successCount++;
//       } else {
//         const name = selectedTemplate === 'paymentReceipt' ? data.studentName : data.recipientName || data.employeeName || 'Unknown';
//         const email = data.email || 'No Email';
//         failedEmails.push(`${name} (${email})`);
//       }
//     });
